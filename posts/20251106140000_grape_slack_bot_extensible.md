...
This event handling integrates with the rest of the framework. Events can access the same user resolution, the same callback storage, the same logging infrastructure. The framework provides consistency across interaction types: commands, interactions, events, and views all use the same patterns.
...
The complexity of Slack's API requirements disappears behind a simple interface. Commands become classes, interactions handlers, events processors, and views builders. The framework handles the details, and you focus on the bot's functionality.
